...
class BlockDiffusionLLM(DiffusionLLM):
    ...
    def __init__(self, model, decoder, iterator_factory, cache_factory, early_stop=True, maximum_unroll=1, expected_tpf=15, backend='vllm', 
                 mini_batch_size=4, prefilling_limit=128, use_naive_batching=True, enable_expert_stats=True, num_layers=None, num_experts=None):
        self.model = model
        self.decoder = decoder
        self.iterator_factory = iterator_factory
        self.cache_factory = cache_factory
        ...
        self.enable_expert_stats = enable_expert_stats
        self.expert_stats = None
        if enable_expert_stats and num_layers is not None and num_experts is not None:
            from dinfer.bd_expert_tracker import BlockDiffusionExpertStatistics
            self.expert_stats = BlockDiffusionExpertStatistics(model, num_layers, num_experts)
    ...
    def dynamic_batching_generate(self, prompt, gen_length=128, block_length=128):
        ...
        self._prefilling_limit = prefilling_limit
