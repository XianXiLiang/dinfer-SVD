...
class BlockDiffusionRunner(BlockRunner):
    ...
    def prefill(self, model, block, kv_cache, pos_ids, attn_mask):
        ...
        if kv_cache is None:
            ...
            print(f"prefill attn_mask {attn_mask.size()}")
            output = model(block, use_cache=True, attention_mask=attn_mask, position_ids=pos_ids)
            print(f"output.past_key_values {output.past_key_values._data[0].size()}")
    ...
    def decode(self, model, decoder, x, kv_cache, block, block_loc, block_id, pos_ids, attn_mask):
        ...
        while (block == decoder.mask_id).sum() > 0:
            ...
            step += 1
            # print(f"step {step} unroll_k {unroll_k} expected_tpf {self.expected_tpf}")
            print(f"past_key_values {past_key_values.get_keys(0).size()}")
            print(f"decode attn_mask {attn_mask.size()}")
            for unroll_i in range(unroll_k):
                ...
                # print(f"step {step} unroll_i {unroll_i} time {(t1-t0)*1e3:1f}")
            if batch_size > 1:
                seq_idx, x = select_undecoded(seq_idx, orig_x, x, block, block_loc, decoder.mask_id, writeback=True)
                block = x[:, block_loc.start:block_loc.end]
                # If all blocks have been decoded, we can jumpt out.
...
class BlockDiffusionLLM(DiffusionLLM):
    ...
    def generate(self, prompt, gen_length=128, block_length=128):
        ...
        # # prepare block_mask and position IDs
        # # mask和fast-dllm v2的图一样
        # # bd_attn_mask的长度是total_length，包含了生成部分
        # block_mask = torch.tril(torch.ones(num_blocks, num_blocks, device=self.model.device))
        # bd_attn_mask = block_mask.repeat_interleave(block_length, dim=0)\
        ...
        prompt_list = []
        position_ids_list = []
        # 我们需要为每个 token 生成两个 ID：
        # req_ids: 属于第几个请求 [0, 0, ..., 1, 1]
        # blk_ids: 属于该请求内的第几个 Block [0, 0, 1, 1, ..., 0, 0, 1, 1]
        req_ids_list = []
        blk_ids_list = []

        new_gen_length = gen_length
        for i, input_id in enumerate(prompt):
            input_id = input_id.squeeze(0)
            input_length = input_id.shape[0]
            num_blocks = (input_length + block_length - 1) // block_length
            new_length = num_blocks * block_length
            new_prompt = torch.zeros((new_length), dtype=torch.long, device=self.model.device).fill_(156895)
            new_prompt[:input_length] = input_id.to(self.model.device)
            prompt_list.append(new_prompt)

            # 生成Mask 1. Request ID: 当前请求全是 i
            req_ids_list.append(torch.full((new_length,), i, device=self.model.device))
            # 2. Block ID: 生成 [0, 1, 2...] 然后扩展到每个 Block 的长度
            # 结果类似: [0,0,..,0, 1,1,..,1, 2,2,..,2]
            b_ids = torch.arange(num_blocks, device=self.model.device)
            b_ids = b_ids.repeat_interleave(block_length) 
            blk_ids_list.append(b_ids)

            pos_id = torch.arange(new_length, dtype=torch.long, device=self.model.device)
            position_ids_list.append(pos_id)

        prompt = torch.cat(prompt_list, dim=0)
        req_ids = torch.cat(req_ids_list, dim=0)          
        blk_ids = torch.cat(blk_ids_list, dim=0)           

        # === 2. 生成 Mask (广播机制) ===
        # 扩充维度以支持广播 [N, 1] vs [1, N]
        req_q, req_k = req_ids.unsqueeze(1), req_ids.unsqueeze(0)
        blk_q, blk_k = blk_ids.unsqueeze(1), blk_ids.unsqueeze(0)
        # 条件 A: 必须是同一个请求 (Request Isolation)
        mask_req = (req_q == req_k)
        # 条件 B: 只有 Key 的 Block ID <= Query 的 Block ID 才能看 (Block Causal)
        mask_block = (blk_k <= blk_q)
        bd_attn_mask = mask_req & mask_block
        prompt_len = prompt.shape[0]
        prompt = prompt.unsqueeze(0)
        bd_attn_mask = bd_attn_mask.unsqueeze(0)
        pos_ids = torch.cat(position_ids_list, dim=0).unsqueeze(0)
        print(f"init bd_attn_mask {bd_attn_mask.size()}")
        ...
        self.block_runner.prefill(self.model, x[:, :prompt_len], kv_cache, pos_ids[:, :prompt_len], bd_attn_mask)
        print(f"Prefill Down")
