...
class KVCache:
    ...
    def update(self, key_states, val_states, layer_idx, replace_position=None):
        ...
        if replace_position is not None:
            torch.cuda.nvtx.range_push(f"KV scatter")
            # print(f"self.get_keys(layer_idx) size {self.get_keys(layer_idx).size()}")
            ...
            # keys = self.get_keys(layer_idx)
            # values = self.get_values(layer_idx)
            # keys[:, :, replace_position[0]:replace_position[1], :].copy_(key_states)
            # values[:, :, replace_position[0]:replace_position[1], :].copy_(val_states)
...
class BlockDiffusionPrefixCacheManager(DiffusionKVCacheManager):
    ...
    def extend_cache(self, end):
        ...
        req_num = len(self.past_key_values._reqlen)
