...
# from dinfer.model.sglangFusedMoE import FusedMoE
...
class LLaDA2MLP(nn.Module):
    def __init__(
        ...
        # else:
        #     rank = config.hidden_size * config.moe_intermediate_size // (config.hidden_size + config.moe_intermediate_size)
        #     # print("rank for svd in LLaDA2MoeMLP:", rank)
        #     # tp_size = get_tensor_model_parallel_world_size()
        #     # print("intermediate: ",config.intermediate_size," hidden: ", config.hidden_size)
        #     self.gate_proj_u = nn.Linear(rank, config.moe_intermediate_size, bias=False)
        #     self.gate_proj_v = nn.Linear(config.hidden_size, rank, bias=False)
        #     self.up_proj_u = nn.Linear(rank, config.moe_intermediate_size, bias=False)
        #     self.up_proj_v = nn.Linear(config.hidden_size, rank, bias=False)
        #     self.down_proj_u = nn.Linear(rank, config.hidden_size, bias=False)
        #     self.down_proj_v = nn.Linear(config.moe_intermediate_size, rank, bias=False)
    ...
    def forward(
        ...
        return hidden_states
        # else:
        #     gate_out = self.gate_proj_u(self.gate_proj_v(hidden_states))
        #     up_out = self.up_proj_u(self.up_proj_v(hidden_states))
        #     down_in = self.act_fn(gate_out) * up_out
        #     return self.down_proj_u(self.down_proj_v(down_in))
...
class LLaDA2SparseMoeBlock(nn.Module):
    def __init__(
        ...
        # print("num_shared_experts:", self.num_shared_experts)
        ...
        # print("intermediate_size:", config.moe_intermediate_size)
        # if not self.use_svd:
        ...
        self._last_topk_ids = None
        self.start = torch.cuda.Event(enable_timing=True)
        self.end = torch.cuda.Event(enable_timing=True)

        # if self.use_svd:
        #     # acquire S
        #     cov = self.profile_covariance(calib_x)
        #     S = torch.linalg.cholesky(cov)
        #     profiling_mat = {expert_id: S for expert_id in range(self.num_experts)}
        #     self.compress_experts(profiling_mat, ratio=0.2)
    ...
    def _forward_router_experts(self, hidden_states: torch.Tensor):
        ...
        self._last_topk_ids = topk_output.topk_ids.detach()

        # from fvcore.nn import flop_count
        # flops_dict, _ = flop_count(self.experts,(hidden_states, topk_output))
        # print(f"FLOPs: {flops_dict.values()}")
        ...
        # duration_ms = self.start.elapsed_time(self.end)
        # print(f"FusedMoE GPU time = {duration_ms * 1e3:.2f} µs")
        # print(f"{duration_ms * 1e3:.2f}")
    ...
    def forward_normal(
        ...
        if (
            ...
            # start.record()
            # torch.cuda.nvtx.range_push(f"FusedMoE")
    ...
    # def get_num_activated_experts(self) -> int:
    #     """
    #     get the num of activated experts in a layer
    #     """
    #     if self._last_topk_ids is None:
    #         return 0
        
    #     with torch.no_grad():
    #         idx = self._last_topk_ids.flatten().cpu().numpy()
    #         mask = idx >= 0
    #         unique_experts = len(np.unique(idx[mask]))
    #         return int(unique_experts)
        
        # return torch.unique(self._last_topk_ids[self._last_topk_ids >= 0]).numel()

    # def reset_activated_experts(self):
    #     """
    #     reset activated experts record
    #     """
    #     self._last_topk_ids = None

    # def profile_covariance(self, x):
    #     """
    #     x: [num_tokens, hidden_size]
    #     return: Σx = x^T x  [hidden, hidden]
    #     """
    #     return torch.matmul(x.t(), x)

    # @torch.no_grad()
    # def compress_one_expert(self, W: torch.Tensor, ratio: float, S: torch.Tensor):
    #     """
    #     W: w13 or w2
    #     ratio: parameter retention ratio
    #     S: [hidden, hidden]
    #     return: (U', V')
    #     """
    #     W_f = W.float()
    #     W_s = W_f @ S                           
    #     U, Sigma, V = torch.linalg.svd(W_s, full_matrices=False)
    #     # r = int(W.shape[0] * W.shape[1] * ratio / (W.shape[0] + W.shape[1]))
    #     r = 320
    #     U_truncated, S_truncated, V_truncated = U[:, :r], Sigma[:r], V[:r, :]
    #     sqrt_S = torch.diag(torch.sqrt(S_truncated))
    #     V_truncated = V_truncated @ torch.linalg.inv(sqrt_S)
    #     U_low = U_truncated @ sqrt_S
    #     V_low = sqrt_S @ V_truncated
    #     return U_low.to(W.dtype), V_low.to(W.dtype)

    # def compress_experts(self, profiling_mat: Dict[int, torch.Tensor], ratio: float):
    #     """
    #     profiling_mat:  dict  expert_id -> S
    #     ratio: parameter retention ratio
    #     return w1(gate), w2(down), w3(up) low-rank factors
    #     """
    #     # FusedMoE shape：
    #     # w13_weight  [E, 2*I, H]  (gate+up)
    #     # w2_weight   [E, H, I]
    #     E = self.num_experts
    #     H = self.hidden_size
    #     I = self.config.moe_intermediate_size

    #     w13 = self.experts.w13_weight.data.view(E, 2*I, H)
    #     w2  = self.experts.w2_weight.data.view(E, H, I)

    #     new_w13, new_w2 = [], []
    #     V13_list, V2_list = [], []

    #     for expert_id in range(E):
    #         S = profiling_mat[expert_id].to(w13.device)
    #         # gate_proj
    #         w1 = w13[expert_id, :I, :]                  # [I, H]
    #         u1, v1 = self.compress_one_expert(w1, ratio, S)
    #         # up_proj
    #         w3 = w13[expert_id, I:, :]                    # [I, H]
    #         u3, v3 = self.compress_one_expert(w3, ratio, S)
    #         # down_proj
    #         w2e = w2[expert_id, :, :]                     # [H, I]
    #         u2, v2 = self.compress_one_expert(w2e, ratio, S)
    #         # back to FusedMoE
    #         new_w13.append(torch.cat([u1, u3], dim=0))  # [2*I, r]
    #         new_w2.append(u2)                           # [H, r]
    #         V13_list.append(torch.cat([v1, v3], dim=0)) # [2I, r]
    #         V2_list.append(v2)                         # [I, r]

    #     r = new_w2[0].shape[1]
    #     # r = new_w13[0].shape[1]
    #     self.experts.w13_weight = nn.Parameter(torch.stack(new_w13, dim=0).contiguous())
    #     self.experts.w2_weight  = nn.Parameter(torch.stack(new_w2,  dim=0).contiguous())
    #     # register V matrices
    #     self.register_parameter('w13_V', nn.Parameter(torch.stack(V13_list, dim=0).contiguous()))
    #     self.register_parameter('w2_V',  nn.Parameter(torch.stack(V2_list,  dim=0).contiguous()))
    #     # cleanup
    #     del w13, w2, new_w13, new_w2, V13_list, V2_list
...
class LLaDA2Attention(nn.Module):
    ...
    def forward(
        ...
        # print(positions.dtype, q.dtype, k.dtype)
        q, k = self.rotary_emb(
            positions.flatten(),
            q.flatten(0, 1),
            k.flatten(0, 1),
        ...
        torch.cuda.nvtx.range_push(f"sdpa") 
...
class LLaDA2Block(nn.Module):
    ...
    def _is_layer_sparse(
        ...
        return is_nextn or (
            ...
            # config.num_experts is not None and layer_id >= getattr(config, 'first_k_dense_replace', 0)
        )
...
class LLaDA2Model(nn.Module):
    ...
    def forward(
        ...
        use_cache: Optional[bool] = None,
        attention_mask: Optional[torch.Tensor]=None,
        **kwargs # for moe statistics
    ) -> Union[torch.Tensor, PPProxyTensors]:

        # for layer in self.layers:
        #     if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'reset_activated_experts'):
        #         layer.mlp.reset_activated_experts()
        ...
        # moe_stats = None
        # if kwargs.get('collect_moe_stats', False):
        #     # torch.cuda.synchronize()
        #     moe_stats = {}
        #     for idx, layer in enumerate(self.layers):
        #         if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'get_num_activated_experts'):
        #             # num_activated = layer.mlp.get_num_activated_experts().item()
        #             num_activated = layer.mlp.get_num_activated_experts()
        #             moe_stats[f'layer_{idx}'] = num_activated
        #             # print(f'[MOE Stats] Layer {idx}: Activated Experts: {num_activated}')
        #             # print(f'{num_activated}')
                    
        #     # print('[MOE Stats]')
        #     kwargs['_moe_activation_stats'] = moe_stats
