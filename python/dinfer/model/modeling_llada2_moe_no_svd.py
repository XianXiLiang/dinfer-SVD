...
class LLaDA2MoeMLP(nn.Module):
    def __init__(self, config: LLaDA2MoeConfig, intermediate_size: int):
        ...
        self.use_svd = False
        if self.use_svd:
            rank = getattr(config, 'svd_rank', min(320, intermediate_size // 2))
            self.gate_proj_u = nn.Linear(rank, self.intermediate_size, bias=False)
            self.gate_proj_v = nn.Linear(self.hidden_size, rank, bias=False)
            self.up_proj_u = nn.Linear(rank, self.intermediate_size, bias=False)
            self.up_proj_v = nn.Linear(self.hidden_size, rank, bias=False)
            self.down_proj_u = nn.Linear(rank, self.hidden_size, bias=False)
            self.down_proj_v = nn.Linear(self.intermediate_size, rank, bias=False)
    ...
    def forward(self, x):
        ...
        if self.use_svd:
            gate_out = self.gate_proj_u(self.gate_proj_v(x))
            up_out = self.up_proj_u(self.up_proj_v(x))
...
class LLaDA2MoeGate(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("_activated_experts", torch.zeros(self.num_experts, dtype=torch.bool), persistent=False)
        self.register_buffer("_last_topk_idx", torch.zeros((1, self.top_k), dtype=torch.long), persistent=False)
    ...
    def forward(self, hidden_states):
        ...
        self._activated_experts[unique_experts] = True
    ...
    def get_num_activated_experts(self):
        """返回本次forward中激活的专家总数"""
        return int(self._activated_experts.sum().item())
    
    def reset_activated_experts(self):
        """重置激活专家统计"""
        self._activated_experts.fill_(False)
...
class LLaDA2MoeSparseMoeBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_experts = config.num_experts
        print("num experts: ", self.num_experts)
        self.top_k = config.num_experts_per_tok
        ...
        self.register_buffer("_total_activated_experts", torch.zeros(self.num_experts, dtype=torch.bool), persistent=False)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(self.gate, '_last_topk_idx'):
            topk_idx = self.gate._last_topk_idx
            # print("topk_idx",topk_idx.shape)
            unique_experts = torch.unique(topk_idx)
            self._total_activated_experts[unique_experts] = True

        return y

    def get_num_activated_experts(self):
        """返回此层累计激活的专家总数"""
        return int(self._total_activated_experts.sum().item())
    
    def reset_activated_experts(self):
        """重置激活专家统计"""
        self._total_activated_experts.fill_(False)
        self.gate.reset_activated_experts()
...
class LLaDA2MoeFlashAttention2(LLaDA2MoeAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("LOG using flash attention")
...
class LLaDA2MoeSdpaAttention(LLaDA2MoeAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # torch.cuda.nvtx.range_push("KV update")
        # print(f"type {type(past_key_value)}")
        # print(f"replace_position {replace_position}")
        ...
        # print(f"LOG using SDPA")
        # print(f"qkv_states {query_states.size()} {key_states.size()} {value_states.size()}")

        # if attention_mask is not None:
        #     print(f"forward attention_mask {attention_mask.size()}")
        # decode不用attention mask
        ...
        if kwargs.get('collect_moe_stats', False):
            ...
            for idx, layer in enumerate(self.layers):
                if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'get_num_activated_experts'):
                    moe_stats[f'layer_{idx}'] = layer.mlp.get_num_activated_experts()
                    # print(f"Layer {idx}: {layer.mlp.get_num_activated_experts()} experts")
                    print(f"{idx}: {layer.mlp.get_num_activated_experts()}")
                    # print(layer.mlp.get_num_activated_experts())
