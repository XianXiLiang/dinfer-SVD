...
model_path='/shared_LLM_model/LLaDA/LLaDA2.0-mini-SVD-Lora-0.6-revert-fixed' # your model path  
...
gpus='4;5;6;7' # gpus for tensor parallel inference
...
if [ parallel=='tp' ]; then
  ...
  for task in gsm8k_llada_mini; do
    # task_yaml="/workspace/dInfer/evaluations/tasks/gsm8k/gsm8k-llada-mini.yaml"
  # for task in gsm8k_llada_mini; do
    ...
    --output_path ${output_path} --include_path ./tasks --apply_chat_template --limit 200
  done
else
