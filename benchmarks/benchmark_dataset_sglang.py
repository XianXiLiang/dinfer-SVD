...
def main(world_size, rank, gpu_id, args):
    ...
    model = ModelRunner(model, device, server_args=server_args, max_length=max_length, prefill_lengths=aligned_lengths, enable_cuda_graph=False, supported_batch_sizes=supported_batch_sizes, use_cross_block=args.batch_size==1)
    ...
    if rank==0:
        ...
        if hasattr(dllm, 'expert_stats') and dllm.expert_stats is not None:
            ...
            dllm.expert_stats.print_block_summary()
            
            expert_stats_dir = os.path.join(args.output_dir, 'expert_activation_stats')
            os.makedirs(expert_stats_dir, exist_ok=True)
            dllm.expert_stats.export_to_csv(expert_stats_dir)
            print(f"✓ Expert statistics exported to {expert_stats_dir}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_gsm8k', action='store_true', help='使用GSM8K数据集进行评测')
    parser.add_argument('--gsm8k_samples', type=int, default=100, help='GSM8K样本数')
