...
# 注意model LLaDAMoeModelLM 在两个版本中命名不同
    ...
    with set_current_vllm_config(VllmConfig(parallel_config = parallel_config)):
        ...
        for wi in range(1):
            ...
            for i in iterator:   
                ...
                # print(f"inner {inner_start-start}")
                ...
                print("end")
                tag2 = time.time()
                # print(f"generate time {tag2-tag1}")
                # print(f"time {tag1-start}")
                # print(f"origin {tag1-origin}")
                ...
                fps = nfe/sample_time
                # prof.export_chrome_trace(f"profiler_{i}.json")
                if rank == 0:
                    print(f'[iter {i:4d}]nfe={nfe:4d}, token number={batch_token_number:4d}, fps={fps:4.2f},tpf={tpf:2.2f}, tps={tps:4.2f}, time={sample_time:4.2f}s')
                    if wi==0 and i<5:
                        for j in range(input_ids.shape[0]):
