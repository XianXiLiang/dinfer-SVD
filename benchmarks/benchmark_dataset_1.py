...
def main(world_size, rank, gpu_id, args):
    ...
    with set_current_vllm_config(VllmConfig(parallel_config = parallel_config)):
        ...
        # all_tokens = list(chain.from_iterable(ids[0].tolist() for ids in all_input_ids))
        # input_ids  = torch.tensor([all_tokens[:512]], dtype=torch.long, device=device)
        ...
        # if rank == 0:
        #     print(f'prompt_len={prompt_len}, time={elapsed:.4f}s, '
        #           f'prefill throughput={throughput:.2f} token/s')
        ...
        for wi in range(1):
            ...
            for i in iterator:   
                ...
                input_ids = [ids[:32] for ids in input_ids]
                max_length = 0
                min_padded_length = 10000
                for j, seq in enumerate(input_ids):
                    # token_number = seq.shape[1]
        ...
        if rank==0:
            ...
            # print(f"Time:{stop-start} TPS:{total_token/(stop-start)}")
